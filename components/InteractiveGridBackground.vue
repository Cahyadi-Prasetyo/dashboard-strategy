...
function getRectClass(index: number) {
  const baseClass =
    "stroke-blue-400/20 transition-all duration-100 ease-in-out [&:not(:hover)]:duration-1000";
  const fill =
    hoveredSquare.value === index ? "fill-blue-300/20" : "fill-transparent";
  return props.squaresClassName
    ? `${baseClass} ${fill} ${props.squaresClassName}`
    : `${baseClass} ${fill}`;
}
