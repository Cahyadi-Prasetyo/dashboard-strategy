<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div class="mb-6">
          <h4 class="text-base font-bold text-gray-900 flex items-center gap-2">
            ...
            />
            Komposisi Status Dokumen Infra
          </h4>
      ...
      <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div class="mb-6">
          ...
          <p class="text-xs text-gray-500 mt-1">
            Breakdown jumlah dokumen infrastruktur
          </p>
        </div>
    ...
    <div>
      <ChartsDataTable
        :tableTitle="`Detail Data Infrastruktur per ${levelLabel}`"
...
const indicators = [
  "progres_infra_open",
  "progres_infra_draft",
  "progres_infra_submitted",
  "progres_infra_approved_pml",
  "progres_infra_rejected_pml",
  "progres_infra_revoked_pml",
];
...
const summaryData = computed(() => {
  const open = getVal("progres_infra_open");
  const draft = getVal("progres_infra_draft");
  const submitted = getVal("progres_infra_submitted");
  const approved = getVal("progres_infra_approved_pml");
  const rejected = getVal("progres_infra_rejected_pml");
  const revoked = getVal("progres_infra_revoked_pml");
...
const chartStatusDokumen = computed(() => {
  return [
    { name: "Open", value: getVal("progres_infra_open"), color: "#3B82F6" },
    { name: "Draft", value: getVal("progres_infra_draft"), color: "#9CA3AF" },
    {
      name: "Submitted",
      value: getVal("progres_infra_submitted"),
      color: "#F59E0B",
    },
    {
      name: "Approved",
      value: getVal("progres_infra_approved_pml"),
      color: "#10B981",
    },
    {
      name: "Rejected",
      value: getVal("progres_infra_rejected_pml"),
      color: "#EF4444",
    },
    {
      name: "Revoked",
      value: getVal("progres_infra_revoked_pml"),
      color: "#1F2937",
    },
  ].filter((i) => i.value > 0);
});
...
const processedDataWilayah = computed(() => {
  ...
  return dataWilayah.value.map((w) => {
    const open = w.progres_infra_open || 0;
    const draft = w.progres_infra_draft || 0;
    const submitted = w.progres_infra_submitted || 0;
    const approved = w.progres_infra_approved_pml || 0;
    const rejected = w.progres_infra_rejected_pml || 0;
    const revoked = w.progres_infra_revoked_pml || 0;
    const target = open + draft + submitted + approved + rejected + revoked;
