<template>
    ...
    >
        ...
        <!-- <div class="mt-8">
            <h3 class="text-lg font-bold text-gray-800 mb-4">
                Distribusi Kerusakan Jalan berdasarkan Tingkat Kerusakan
            </h3>
        ...
        <!-- Chart Modal - Dikomen, jika tidak digunakan lagi -->
        <Teleport to="body">
            <div
                v-if="isChartOpen"
                ...
                >
                    ...
                    >
                        <h3 class="text-lg font-bold text-gray-800">
                            Distribusi Kerusakan Jalan berdasarkan Tingkat
                            Kerusakan
                        </h3>
...
import JalanDetailModal from "~/components/infrastruktur/JalanDetailModal.vue";
...
// Jenis jalan yang akan ditampilkan
const jenisJalan = [
    {
        key: "Jalan Nasional",
        label: "Panjang Jalan Nasional Rusak",
        color: "red",
        icon: "i-heroicons-map",
    },
    {
        key: "Jalan Provinsi",
        label: "Panjang Jalan Provinsi Rusak",
        color: "red",
        icon: "i-heroicons-map",
    },
    {
        key: "Jalan Kabupaten",
        label: "Panjang Jalan Kabupaten Rusak",
        color: "red",
        icon: "i-heroicons-map",
    },
];
...
async function fetchJalanData() {
    ...
    try {
        ...
        // Filter hanya data jalan (bukan jembatan)
        rawData.value = allRecords.filter((record) => {
            const nama = record.itemCategory2?.nama || "";
            return nama.toLowerCase().includes("jalan");
        });
...
const summaryCards = computed(() => {
    return jenisJalan.map((jenis) => {
        // Calculate total for this jenis jalan
        const totalForJenis = rawData.value
            .filter((r) => r.itemCategory2?.nama === jenis.key)
            .reduce((sum, r) => sum + (Number(r.value) || 0), 0);

        // Calculate breakdown by damage type
...
const chartCategories = computed(() => {
    return jenisJalan.map((j) => j.label);
});

const chartSeries = computed(() => {
    return statusKerusakan.map((status) => {
        const data = jenisJalan.map((jenis) => {
