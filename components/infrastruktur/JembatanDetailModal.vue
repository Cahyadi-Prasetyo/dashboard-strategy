                ...
                <div class="flex items-center gap-2">
                    <div class="w-64">
                        <input
                            ...
                            class="w-full pl-9 pr-4 py-1 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder-gray-500 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all"
                        />
                    </div>
                        ...
                        <template #row="{ row, index, startIndex }">
                            ...
                            <td
                                ...
                                :class="canDrillDown(row)
                                    ? 'text-slate-700 cursor-pointer hover:bg-purple-50 hover:underline'
                                    : 'text-slate-900'"
                            ...
                            <td class="px-3 py-2 text-[12px] text-right font-bold text-purple-700 bg-purple-50/50">
...
// Tabs
const tabs = [
    { label: 'Jembatan Nasional', key: 'Jembatan Nasional' },
    { label: 'Jembatan Provinsi', key: 'Jembatan Provinsi' },
    { label: 'Jembatan Kabupaten', key: 'Jembatan Kabupaten' },
];
...
const currentTitle = computed(() => {
    return currentParent.value
        ? `Detail ${tabs[activeTab.value].label} - ${currentParent.value.name}`
        : `Kerusakan ${tabs[activeTab.value].label}`;
});
...
const tableColumns = computed(() => {
    ...
    statusKerusakan.forEach(status => {
        cols.push({ 
            key: status.key, 
            label: `${status.label} (KM)`, 
            align: "right" 
        });
    });
    
    cols.push({ key: "total", label: "Total (KM)", align: "right" });
...
const processedData = computed(() => {
    ...
    const currentJenisJembatan = tabs[activeTab.value].key;
    const groupedMap = new Map();

    rawRecords.value.forEach((r) => {
        // Filter by current tab
        if (r.itemCategory2?.nama !== currentJenisJembatan) return;
...
const triggerDownload = async () => {
    ...
    try {
        ...
        if (typeof downloadDynamicExcel === "function") {
            const fileName = `Kerusakan_${tabs[activeTab.value].label.replace(/\s+/g, '_')}_${new Date().toISOString().split("T")[0]}`;
