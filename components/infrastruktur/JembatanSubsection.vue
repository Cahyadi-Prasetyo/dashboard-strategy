<template>
    ...
    >
        ...
        >
            <div class="flex flex-wrap items-center gap-2">
                ...
                >
                    ...
                    />
                    <span
                        class="text-[12px] font-black text-amber-700 uppercase tracking-wider"
                    >
                        INFRASTRUKTUR JEMBATAN TERDAMPAK
                    </span>
                </div>
            </div>
                        ...
                        <template #default>
                            ...
                            >
                                <div class="flex items-baseline gap-2">
                                    <span
                                        :class="`text-4xl font-extrabold tracking-tight text-${item.color}-700`"
                                    >
                                        {{
                                            item.total == null
                                                ? "N/A"
                                ...
                                >
                                    <div
                                        v-for="detail in item.breakdown"
                                        :key="detail.key"
                                ...
                                <SharedDatasetInfoCard
                                    :source="metaData.dataset?.sumber"
                                    :last-update="
                                        metaData.dataset?.terakhir_diupdate
                                    "
                                />
                            </div>
                        </template>
        ...
        <!-- Chart Section - Langsung di bawah big numbers -->
        <!-- <div class="mt-8">
            <h3 class="text-lg font-bold text-gray-800 mb-4">
                Distribusi Kerusakan Jembatan berdasarkan Tingkat Kerusakan
            </h3>
        ...
        <!-- Chart Modal - Dikomen, tidak digunakan lagi  -->
        <Teleport to="body">
            <div
                v-if="isChartOpen"
                ...
                >
                    ...
                    >
                        <h3 class="text-lg font-bold text-gray-800">
                            Distribusi Kerusakan Jembatan berdasarkan Tingkat
                            Kerusakan
                        </h3>
...
import JembatanDetailModal from "~/components/infrastruktur/JembatanDetailModal.vue";
...
// Jenis jembatan yang akan ditampilkan
const jenisJembatan = [
    {
        key: "Jembatan Nasional",
        label: "Panjang Jembatan Nasional Rusak",
        color: "red",
        icon: "i-heroicons-rectangle-group",
    },
    {
        key: "Jembatan Provinsi",
        label: "Panjang Jembatan Provinsi Rusak",
        color: "red",
        icon: "i-heroicons-rectangle-group",
    },
    {
        key: "Jembatan Kabupaten",
        label: "Panjang Jembatan Kabupaten Rusak",
        color: "red",
        icon: "i-heroicons-rectangle-group",
    },
];
...
const statusKerusakan = [
    ...
    { key: "Rusak Tidak Diketahui", color: "#6B7280" },
];

// Tambahkan setelah line import
const damageList = [
    {
        key: "rusakringan",
        label: "Rusak Ringan",
...
// GANTI formatDate dengan formatNumber:
...
// Fetch data
async function fetchJembatanData() {
    loading.value = true;
    try {
        const w = props.selectedWilayah;
        ...
        // Filter hanya data jembatan (bukan jalan)
        rawData.value = allRecords.filter((record) => {
            const nama = record.itemCategory2?.nama || "";
            return nama.toLowerCase().includes("jembatan");
        });
        ...
        console.error("Error fetching jembatan data:", error);
        rawData.value = [];
    } finally {
        loading.value = false;
    }
...
// Summary cards
// GANTI computed summaryCards yang lama dengan ini:
const summaryCards = computed(() => {
    return jenisJembatan.map((jenis) => {
        // Calculate total for this jenis jembatan
        const totalForJenis = rawData.value
            .filter((r) => r.itemCategory2?.nama === jenis.key)
        ...
        const breakdown = damageList.map((damage) => {
            const total = rawData.value
                .filter(
                    (r) =>
                        r.itemCategory2?.nama === jenis.key &&
                        r.itemCategory4?.nama === damage.label,
                )
                .reduce((sum, r) => sum + (Number(r.value) || 0), 0);
...
const chartCategories = computed(() => {
    return jenisJembatan.map((j) => j.label);
});

const chartSeries = computed(() => {
    return statusKerusakan.map((status) => {
        const data = jenisJembatan.map((jenis) => {
            return rawData.value
                .filter(
                    (r) =>
                        r.itemCategory2?.nama === jenis.key &&
                        r.itemCategory4?.nama === status.key,
                )
                .reduce((sum, r) => sum + (Number(r.value) || 0), 0);
        });
