<template>
    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all relative">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="font-bold text-gray-800 text-sm">
                        ...
                        Atap Terluas Rumah yang Ditempati Setelah Bencana
                    </h4>
            ...
            <!-- Filter Jenis Atap -->
            <div class="w-48">
                <USelectMenu class="w-full" v-model="selectedJenisAtap" :items="jenisAtapOptions"
                    placeholder="Pilih Jenis Atap" value-attribute="value" option-attribute="label">
                    <template #label>
                        <span class="flex items-center gap-2">
                            <UIcon :name="selectedJenisAtap.icon" class="w-4 h-4" />
                            {{ selectedJenisAtap.label }}
                        </span>
                    </template>
        ...
        <div class="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-lg">
            <div class="flex items-start gap-2">
                ...
                <div class="text-xs text-blue-800">
                    ...
                    <p>
                        Menampilkan jumlah keluarga yang bangunan utama atap terluasnya adalah
                        <strong>{{ selectedJenisAtap.label }}</strong>
                        berdasarkan kondisi atap tersebut (Baik, Sebagian Kecil Lepas,
                        Sebagian Rangka Patah, Jatuh/Tidak Ada Atap).
                    </p>
                </div>
            </div>
        </div>
...
const jenisAtapOptions = [
    ...
    },
    {
        ...
        prefix: "atap_kayu_sirap",
    },
];
const selectedJenisAtap = ref(jenisAtapOptions[0]);

// --- GENERATE INDICATOR IDS BASED ON SELECTED TYPE ---
const currentIndicators = computed(() => {
    const prefix = selectedJenisAtap.value.prefix;
    return [
        `${prefix}_baik`,
        `${prefix}_lepas_sebagian_kecil`,
        `${prefix}_lepas_sebagian_besar`,
        `${prefix}_lepas_sebagian_seluruh`,
    ];
});
...
// Watch for changes in regionIds or selectedJenisAtap
watch(
    [() => props.regionIds, selectedJenisAtap],
    () => {
        refresh();
    },
